...
class EPGSelection(Screen, HelpableScreen):
		...
		if not InfoBarInstance.LongButtonPressed:
			if self.type == EPG_TYPE_GRAPH:
				if self.changeText != self.EMPTY:
					#Option dialog will be displayed if graphical EPG is in view plus an event is present (changeText variable is not empty)
					self.OptionDialog()
				else:
					self.openEPGSearch()
			else:
	...
	def RecordTimerQuestion(self, manual=False):
		...
		for timer in self.session.nav.RecordTimer.timer_list:
			...
			if not manual:
				menu = [(_("Add Timer"), 'CALLFUNC', self.ChoiceBoxCB, self.doRecordTimer), (_("Add AutoTimer"), 'CALLFUNC', self.ChoiceBoxCB, self.addAutoTimer)]
	...
	def onSelectionChanged(self):
		...
		if self.type == EPG_TYPE_GRAPH:
			...
			if isRecordEvent and self.key_green_choice != self.REMOVE_TIMER:
				...
				self['key_green'].setText(_('Record'))
				self.key_green_choice = self.ADD_TIMER
	...
	def OptionDialog(self):
		...
		if self.changeText != self.EMPTY:
			cur = self['list'].getCurrent()
			event = cur[0]		
			menu = [(_("IMDb Search"), 'CALLFUNC', self.ChoiceBoxCB, self.openIMDb), (_(self.changeText), 'CALLFUNC', self.ChoiceBoxCB, self.OptionDialogAddTimer), (_("Add AutoTimer"), 'CALLFUNC', self.ChoiceBoxCB, self.addAutoTimer),  (_("Bouquet list"), 'CALLFUNC', self.ChoiceBoxCB, self.Bouquetlist), (_("Search"), 'CALLFUNC', self.ChoiceBoxCB, self.openEPGSearch), (_("Enter Date Time"), 'CALLFUNC', self.ChoiceBoxCB, self.enterDateTime), (_("Reduce Time Scale"), 'CALLFUNC', self.ChoiceBoxCB, self.ReduceTimeScale), (_("Increase Time Scale"), 'CALLFUNC', self.ChoiceBoxCB, self.IncreaseTimeScale)]
			title = "%s?" % event.getEventName()
			self.ChoiceBoxDialog = self.session.instantiateDialog(ChoiceBox, title=title, list=menu, keys=['1', '2', '3', '4' , '5', '6', '7', '8'], skin_name="OptionDialog")
			serviceref = eServiceReference(str(self['list'].getCurrent()[1]))
			posy = self['list'].getSelectionPosition(serviceref)
